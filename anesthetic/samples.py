...
from anesthetic.utils import compute_nlive, compute_insertion_indexes, is_int, logsumexp
from anesthetic.gui.plot import RunPlotter
from anesthetic.weighted_pandas import WeightedDataFrame, WeightedSeries
from anesthetic.labelled_pandas import LabelledDataFrame, LabelledSeries
from pandas.core.accessor import CachedAccessor
from anesthetic.plot import make_1d_axes, make_2d_axes, AxesSeries, AxesDataFrame
import anesthetic.weighted_pandas
from anesthetic.plotting import PlotAccessor

anesthetic.weighted_pandas._WeightedObject.plot = CachedAccessor("plot", PlotAccessor)
...
class WeightedLabelledDataFrame(WeightedDataFrame, LabelledDataFrame):
    ...
    def set_labels(self, labels, axis=1, inplace=False, level=None):
        ...
        return super().set_labels(labels, axis=axis, inplace=inplace, level=level)
            ...
            for x, ax in row.items():
                if ax is not None:
                    ...
                    if x in self and y in self and lkwargs["kind"] is not None:
                        ...
                        if x == y:
                            self[x].plot(ax=ax.twin, xlabel=xlabel, *args, **lkwargs)
                            ax.set_xlabel(xlabel)
    ...
    * ``self.importance_sample()``
    ...
    def stats(self, nsamples=None, beta=None):
        ...
        if nsamples is None and beta is None:
            samples = self._constructor_sliced(index=self.columns[:0], dtype=float)
        else:
            samples = WeightedLabelledDataFrame(
                index=logw.columns, columns=self.columns[:0]
            )
    ...
    def recompute(self, logL_birth=None, inplace=False):
        ...
        if is_int(logL_birth):
            ...
            if "logL_birth" not in samples:
                raise RuntimeError(
                    "Cannot recompute run without " "birth contours logL_birth."
                )
            ...
            if n_bad:
                warnings.warn(
                    ...
                    "recommended."
                    "\nDropping the invalid samples." % (n_bad, len(samples), n_equal),
                    RuntimeWarning,
                )
...
def merge_samples_weighted(samples, weights=None, label=None):
    ...
    if weights is None:
        try:
            ...
            raise ValueError(
                "If samples includes MCMCSamples " "then weights must be given."
            )
