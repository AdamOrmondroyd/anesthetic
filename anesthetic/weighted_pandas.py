...
class WeightedSeries(_WeightedObject, Series):
    ...
    def var(self, skipna=True):
        ...
        if np.isnan(mean):
            return mean
        return np.average(
            masked_array((self - mean) ** 2, null), weights=self.get_weights()
        )
    ...
    def cov(self, other, skipna=True):
        ...
        if np.isnan(x) or np.isnan(y):
            return np.nan
        return np.average(
            masked_array((self - x) * (other - y), null), weights=self.get_weights()
        )
    ...
    def kurt(self, skipna=True):
        ...
        if np.isnan(mean) or np.isnan(std):
            return np.nan
        return np.average(
            masked_array(((self - mean) / std) ** 4, null), weights=self.get_weights()
        )
    ...
    def skew(self, skipna=True):
        ...
        if np.isnan(mean) or np.isnan(std):
            return np.nan
        return np.average(
            masked_array(((self - mean) / std) ** 3, null), weights=self.get_weights()
        )
    ...
    def mad(self, skipna=True):
        ...
        if np.isnan(mean):
            return np.nan
        return np.average(
            masked_array(abs(self - mean), null), weights=self.get_weights()
        )
...
class WeightedDataFrame(_WeightedObject, DataFrame):
    ...
    def mean(self, axis=0, skipna=True, *args, **kwargs):
        ...
        if self.isweighted(axis):
            null = self.isnull() & skipna
            mean = np.average(
                masked_array(self, null), weights=self.get_weights(axis), axis=axis
            )
    ...
    def cov(self, skipna=True, *args, **kwargs):
        ...
        if self.isweighted():
            ...
            cov = np.ma.dot(self.get_weights() * x.T, x) / self.get_weights().sum().T
    ...
    def corrwith(self, other, axis=0, drop=False, method="pearson", *args, **kwargs):
        """Pairwise weighted pearson correlation."""
        if self.isweighted(axis):
            if isinstance(other, Series):
                answer = self.apply(lambda x: other.corr(x, method=method), axis=axis)
            ...
            if not drop:
                ...
                result_index = self._get_axis(1 - axis).union(other._get_axis(1 - axis))
            ...
            return super().corrwith(
                other, drop=drop, axis=axis, method=method, *args, **kwargs
            )
    ...
    def kurt(self, axis=0, skipna=True, *args, **kwargs):
        ...
        if self.isweighted(axis):
            ...
            kurt = np.average(
                masked_array(((self - mean) / std) ** 4, null),
                weights=self.get_weights(axis),
                axis=axis,
            )
    ...
    def skew(self, axis=0, skipna=True, *args, **kwargs):
        ...
        if self.isweighted(axis):
            ...
            std = self.std(axis=axis, skipna=skipna)
            skew = np.average(
                masked_array(((self - mean) / std) ** 3, null),
                weights=self.get_weights(axis),
                axis=axis,
            )
    ...
    def mad(self, axis=0, skipna=True, *args, **kwargs):
        ...
        if self.isweighted(axis):
            ...
            mean = self.mean(axis=axis, skipna=skipna)
            mad = np.average(
                masked_array(abs(self - mean), null),
                weights=self.get_weights(axis),
                axis=axis,
            )
    ...
    def quantile(
        self, q=0.5, axis=0, numeric_only=None, interpolation="linear", method=None
    ):
        """Weighted quantile of the sampled distribution."""
        if self.isweighted(axis):
            ...
            data = np.array(
                [c.quantile(q=q, interpolation=interpolation) for _, c in self.items()]
            )
            if np.isscalar(q):
                return self._constructor_sliced(data, index=self._get_axis(1 - axis))
            else:
                return self._constructor(
                    data.T, index=q, columns=self._get_axis(1 - axis)
                )
        else:
    ...
    def compress(self, ncompress=True, axis=0):
        ...
        """
        if self.isweighted(axis):
            i = compress_weights(self.get_weights(axis), self._rand(axis), ncompress)
    ...
    def sample(self, *args, **kwargs):
        ...
        if self.isweighted(axis):
            return super().sample(weights=self.get_weights(axis), *args, **kwargs)
        else:
