...
def loglikelihood(x):
    ...
    a, b, m = 2.0, 4.0, 0.5  # x4 parameters
    ...
    logl += np.log(1 / (b - a) + m * (x4 - (b + a) / 2.0))
    return logl
...
roots.append(root)
mcmc[["weight", "minuslogL"] + columns][: len(mcmc) // 2].to_csv(
    root + "_1.txt", sep=" ", index=False, header=False
)
mcmc[["weight", "minuslogL"] + columns][len(mcmc) // 2 :].to_csv(
    root + "_2.txt", sep=" ", index=False, header=False
)

# MCMC single file
root = "./tests/example_data/gd_single"
roots.append(root)
mcmc[["weight", "minuslogL"] + columns].to_csv(
    root + ".txt", sep=" ", index=False, header=False
)
...
def ns_sim(ndims=5, nlive=125):
    ...
    return (
        np.array(dead_points),
        np.array(dead_likes),
        np.array(birth_likes),
        live_points,
        live_likes,
        live_birth_likes,
    )
...
ns = NestedSamples(
    data=data, columns=columns, logL=logL, logL_birth=logL_birth, tex=tex
)
live_ns = NestedSamples(
    data=live, columns=columns, logL=live_logL, logL_birth=live_logL_birth, tex=tex
)
...
ns[columns + ["logL", "logL_birth"]].to_csv(
    root + "_dead-birth.txt", sep=" ", index=False, header=False
)
live_ns[columns + ["logL", "logL_birth"]].to_csv(
    root + "_phys_live-birth.txt", sep=" ", index=False, header=False
)
...
roots.append(root)
ns["dlogX"] = ns.dlogX()
ns[columns + ["logL", "logL_birth", "dlogX", "cluster"]].to_csv(
    root + "dead-birth.txt", sep=" ", index=False, header=False
)
live_ns[columns + ["logL", "logL_birth", "cluster"]].to_csv(
    root + "phys_live-birth.txt", sep=" ", index=False, header=False
)
ns[columns + ["logL", "dlogX", "cluster"]].to_csv(
    root + "ev.dat", sep=" ", index=False, header=False
)
live_ns[columns + ["logL", "cluster"]].to_csv(
    root + "phys_live.points", sep=" ", index=False, header=False
)
...
ns[columns + ["logL", "cluster"]].to_csv(
    root + "phys_live.points", sep=" ", index=False, header=False
)
...
logL_birth, live_logL_birth = np.split(
    np.concatenate([logL_birth, live_logL_birth[i]]), [-1]
)
ns = NestedSamples(
    data=data, columns=columns, logL=logL, logL_birth=logL_birth, tex=tex
)
...
logL_birth, live_logL_birth = np.split(
    np.concatenate([logL_birth, live_logL_birth]), [len(data) + 1]
)
ns = NestedSamples(
    data=data, columns=columns, logL=logL, logL_birth=logL_birth, tex=tex
)
