...
for p in [0.66, 0.95]:
    k = root(lambda k: -2 * np.exp(-k) * np.sqrt(k / np.pi) + erf(np.sqrt(k)) - p, 1).x[
        0
    ]
...
for p in [0.66, 0.95]:
    k = root(
        lambda k: (
            ...
            / (b - a) ** 2
            * (2 + (b - a) ** 2 * m) ** 2
            * erf(np.sqrt(np.log((2 + (b - a) ** 2 * m) / 2 / (b - a) / k)))
            - 4
            * k**2
            * erfi(np.sqrt(np.log((2 + (b - a) ** 2 * m) / 2 / (b - a) / k)))
        )
        / 8
        / m
        - p,
