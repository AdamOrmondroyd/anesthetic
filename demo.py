...
mcmc.plot_2d(
    ["omegabh2", "omegach2", "H0"], kind={"lower": "kde_2d", "diagonal": "kde_1d"}
)
...
mcmc.plot_2d(
    axes.iloc[0:1, :],
    kind=dict(upper="kde_2d", lower="kde_2d", diagonal="kde_1d"),
    fc=None,
)
mcmc.plot_2d(
    axes.iloc[1:2, :],
...
for x in ["logZ", "D_KL", "d_G", "logL_P"]:
    print(
        "%10s = %9.2f +/- %4.2f"
        % (ns_output.get_label(x), ns_output[x].mean(), ns_output[x].std())
    )
