...
def test_LabelledSeries_MultiIndex():
    ...
    multiindex = MultiIndex.from_arrays([vowels, index], names=["vowels", None])
    ...
    assert_series_equal_not_index(lseries.xs("A", level=1), series.xs("A", level=1))
    assert_series_equal_not_index(
        lseries.xs("vowel", level=0), series.xs("vowel", level=0)
    )
    assert_array_equal(
        lseries.xs(("vowel", "A"), level=[0, 1]),
...
def lframe_index():
    ...
    assert_frame_equal_not_index(lframe.loc["A":"C", 0:3], frame.loc["A":"C", 0:3])
    assert_frame_equal_not_index(lframe.loc["A":"C", 0:3], frame.loc["A":"C", 0:3])
...
def test_LabelledDataFrame_index_MultiIndex():
    ...
    assert_series_equal_not_name(
        lframe.loc["vowel", "A", "$A$"], frame.loc["vowel", "A"]
    )

    assert_frame_equal_not_index(lframe.xs("vowel"), frame.xs("vowel"))
    assert_series_equal(lframe.xs(("vowel", "A")), frame.xs(("vowel", "A")))
    assert_series_equal_not_name(
        lframe.xs(("vowel", "A", "$A$")), frame.xs(("vowel", "A"))
    )

    assert_series_equal(lframe.loc[("vowel", "A"), 0], frame.loc[("vowel", "A"), 0])
    assert_series_equal(
        lframe.loc[("vowel", "A", "$A$"), 0], frame.loc[("vowel", "A"), 0]
    )
    ...
    assert_series_equal_not_name(
        lframe.T["vowel", "A", "$A$"], lframe.loc["vowel", "A"]
    )
    assert_frame_equal(lframe.T.vowel, lframe.loc["vowel"].T)
    assert_series_equal_not_name(lframe.T.vowel.A, lframe.loc["vowel", "A"])
    assert_frame_equal(lframe.T.loc[:, "vowel"], lframe.loc["vowel"].T)
    assert_series_equal(lframe.T.loc[:, ("vowel", "A")], lframe.loc[("vowel", "A")])
    assert_series_equal_not_name(
        lframe.T.loc[:, ("vowel", "A", "$A$")], lframe.loc[("vowel", "A")]
    )
    assert_series_equal(lframe.T.loc[0, "vowel"], lframe.loc["vowel", 0])
    assert lframe.T.loc[0, ("vowel", "A")] == lframe.loc[("vowel", "A"), 0]
    assert lframe.T.loc[0, ("vowel", "A", "$A$")] == lframe.loc[("vowel", "A"), 0]

    assert_series_equal(lframe.T.at[0, ("vowel", "A")], lframe.at[("vowel", "A"), 0])
    assert_series_equal(
        lframe.T.at[0, ("vowel", "A", "$A$")], lframe.at[("vowel", "A"), 0]
    )
...
def test_LabelledDataFrame_column():
    ...
    assert_frame_equal_not_index(lframe.loc[0:3, "A":"C"], frame.loc[0:3, "A":"C"])
    assert_frame_equal_not_index(lframe.loc[0:3, "A":"C"], frame.loc[0:3, "A":"C"])
...
def test_LabelledDataFrame_column_MultiIndex():
    ...
    multiindex = MultiIndex.from_arrays([vowels, columns], names=["vowels", None])
    ...
    assert_frame_equal_not_index(lframe["vowel"], frame["vowel"])
    assert_series_equal(lframe["vowel", "A"], frame["vowel", "A"])
    assert_series_equal_not_name(lframe["vowel", "A", "$A$"], frame["vowel", "A"])

    assert_frame_equal_not_index(lframe.xs("vowel", axis=1), frame.xs("vowel", axis=1))
    assert_series_equal(
        lframe.xs(("vowel", "A"), axis=1), frame.xs(("vowel", "A"), axis=1)
    )
    assert_series_equal_not_name(
        lframe.xs(("vowel", "A", "$A$"), axis=1), frame.xs(("vowel", "A"), axis=1)
    )

    assert_series_equal(lframe.loc[0, ("vowel", "A")], frame.loc[0, ("vowel", "A")])
    assert_series_equal(
        lframe.loc[0, ("vowel", "A", "$A$")], frame.loc[0, ("vowel", "A")]
    )
    ...
    assert_series_equal_not_name(
        lframe.T.loc["vowel", "A", "$A$"], lframe["vowel", "A"]
    )
    assert_series_equal_not_name(
        lframe.T.loc[("vowel", "A", "$A$")], lframe[("vowel", "A")]
    )
    ...
    assert lframe.T.loc[("vowel", "A", "$A$"), 0] == lframe.loc[0, ("vowel", "A")]

    assert_series_equal(lframe.T.at[("vowel", "A"), 0], lframe.at[0, ("vowel", "A")])
    assert_series_equal(
        lframe.T.at[("vowel", "A", "$A$"), 0], lframe.at[0, ("vowel", "A")]
    )
...
def test_multiaxis(lframe_index):
    ...
    columns = MultiIndex.from_arrays(
        [["one", "two", "three", "four"], [1, 2, 3, 4]], names=[None, "aliases"]
    )
...
def test_multiaxis_slice(test_multiaxis):
    ...
    assert_series_equal_not_name(lframe.loc[:, "one"], lframe.loc[:, ("one", 1)])

    assert_frame_equal(
        lframe.loc[["A", "B"], ["one", "two"]],
        lframe.loc[[("A", "$A$"), ("B", "$B$")], [("one", 1), ("two", 2)]],
    )

    assert_series_equal_not_name(
        lframe.loc["A", ["one", "two"]],
        lframe.loc[("A", "$A$"), [("one", 1), ("two", 2)]],
    )
    assert_series_equal_not_name(
        lframe.loc[["A", "B"], "one"],
        lframe.loc[[("A", "$A$"), ("B", "$B$")], ("one", 1)],
    )
