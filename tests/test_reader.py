...
def test_read_getdist():
    ...
    os.rename(
        "./tests/example_data/gd.paramnames", "./tests/example_data/gd.paramnames_"
    )
    mcmc = read_getdist("./tests/example_data/gd")
    os.rename(
        "./tests/example_data/gd.paramnames_", "./tests/example_data/gd.paramnames"
    )
...
@pytest.mark.xfail(
    "getdist" not in sys.modules, raises=NameError, reason="requires getdist package"
)
...
def test_read_montepython():
    ...
    w = np.concatenate(
        (np.loadtxt(root + "_1.txt", usecols=0), np.loadtxt(root + "_2.txt", usecols=0))
    )
    params = [
        "x0",
        "x1",
